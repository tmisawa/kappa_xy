...
import read_dirmvmc #using read_dirmvmc.py#
import hphi_io #using read_dirmvmc.py#

Ncond = 16
#[s] read input.txt
list_lat =['Lx','Ly','orb_num'] # list for lattice parametes
...
#[s] allocate
Nq     = np.zeros([Lx+1,Ly+1],dtype=np.float)
Nq_err = np.zeros([Lx+1,Ly+1],dtype=np.float)
Sq     = np.zeros([Lx+1,Ly+1],dtype=np.float)
Sq_err = np.zeros([Lx+1,Ly+1],dtype=np.float)
#[e] allocate
max_num = 1
for num_cg in range(0,max_num):
    file_name = "zvo_cisajscktalt_eigen"+"{0:1d}".format(num_cg)+".dat"
            ...
            for ky in range(0,Ly+1):
                ...
                for cnt in range(0,cnt_max):
                    ...
                    if cnt%6   == 0 or cnt%6 == 3:
                       tmp_Nq +=      float(tmp[8])*math.cos(theta)
