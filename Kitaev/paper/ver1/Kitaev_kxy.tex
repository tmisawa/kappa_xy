...
The thermal Hall conductivity---a thermal current analog of the Hall conductivity for electric current---is a useful detector for exotic quasiparticles, %particles,
 such as the Majorana %particles.
...
remains unclear how the thermal Hall conductivity behaves at finite temperatures even %for 
in the celebrated Kitaev model on a honeycomb lattice, which is one of the basic models for realizing the Majorana %particles 
...
We demonstrate that the additional off-diagonal interactions (namely, $\Gamma$ and $\Gamma^{\prime}$ terms) %largely 
significantly affect the thermal Hall conductivity. In particular, we show that negative $\Gamma^{\prime}$ and positive $\Gamma$ terms %remarkably enhance the amplitude 
...
that does not include quantum effects, as a classical counterpart of quantum systems. From the comparison between the classical systems and the quantum systems, we find that the effects of the $\Gamma^{\prime}$ term can be captured by the classical model while the effects of the $\Gamma$ term cannot. 
This indicates that 
...
the thermal Hall conductivity in Kitaev materials such as $\alpha$-RuCl$_{3}$ and 
...
suggest that applying magnetic fields %induces 
...
It has also shown that %the angular 
magnetic-field angle dependence of the thermal Hall conductivity
on the magnetic field is consistent with %those 
...
For example, the observed thermal Hall conductivity shows an overshoot %of 
beyond the half-quantized value 
for higher temperatures, and the origin of such overshooting behavior has not been resolved  %while 
...
beyond %the 
perturbation theories \cite{TakikawaF2020, YamadaF2021} %is
has not yet been fully clarified. %yet.

Furthermore, several later experiments %for 
...
and weak magnetic fields, which %are introduced 
...
For finite-temperature properties, we may use Monte Carlo simulation %for 
...
magnetic fields, which is necessary for thermal Hall transport, we need to consider an effective interaction %justified only for 
...
have also been investigated based on the perturbative treatment justified in the %situation 
...
The total gap %is increased (decreased)
increases (decreases)
%for 
...
this suggests that the effect of $\Gamma$ in the thermal Hall transport is %less 
weaker than that of $\Gamma'$, 
the same analysis also showed a possible quantum phase transition induced by the effective interaction arising from %the 
third-order perturbation. 
Therefore, a non-trivial effect may be induced by %the 
such phase transitions.
Note that, as we mentioned, these analyses are based on %the
...
in the zero-temperature limit. 
Thus, to understand the effect of the off-diagonal interactions at finite temperatures, it is necessary to perform an analysis beyond %the 
perturbation theory.
...
We find that %this 
the calculated thermal Hall conductivity shows a clear overshooting behavior similar to the experimental observations, indicating that the experimentally observed thermal Hall conductivity might be explained by %the 
simple extended Kitaev models.
We also %discuss 
analyze the effect of off-diagonal interactions on the thermal Hall conductivity. 
Indeed, we find that depending on the sign and %the 
type of %the 
...
The Hamiltonian of the model under a magnetic field is given %as
by
\begin{equation}
...
The off-diagonal symmetric spin interactions %with 
denoted by $\Gamma$ and $\Gamma'$ were %first introduced
...
The $\Gamma'$ interaction does not appear %in the case where
when the octahedra surrounding transition metal ions retain the cubic symmetry.
The symmetry lowering by a trigonal distortion, which is inevitable in real materials, %results in 
...
Most theoretical works have suggested $K<0$ and $\Gamma>0$ for $\alpha$-RuCl$_3$ %but 
although the sign of $\Gamma'$ %is still 
remains under debate.

Because of the complexity of realistic models, the $K$-$\Gamma$ model has been recently studied %recently 
...
Once quantum fluctuations are taken into account, quantum-disordered states could %potentially 
emerge, but the detailed phase diagram %is still 
...
It has been pointed out that a ferromagnetic phase and chiral spin phase with nonzero spin scalar chirality appear %in the case with
...
Thus, this model might be suitable %to investigate 
...
Throughout the paper, we consider the ferromagnetic Kitaev interaction $K= -1$, which %is naturally expected 
...
lattice to the unit length and $k_B = \hbar = 1$. We typically discuss %, typically, 
physical quantities 
for the model %on 
with $(L, L') = (6, 6)$ shown in Fig.~\ref{fig:lattice}(a). The total system size is %given by 
...
the thermal Hall conductivity for finite-size clusters described above. Then, we %explain %three 
describe three
...
In the Majorana representation, the effective field %gives 
...
Thus, the chiral edge current originating from $L_{ijk}^{\gamma\gamma'}$ is expected to %appear 
...
%Since the 
The energy current in the system should be defined %by 
...
Here, we introduce the energy current $J_{{\rm E},l}^\parallel$ along the zigzag chain labeled by line $l$, such that %$J_{{\rm E},l}^\parallel$ 
it includes the contributions from the local currents on the segments inside %the 
...
Note that due to the symmetry of the system, %$J_{\mathrm{E}}$ 
 ...
 {$J_{\mathrm{E}}^{\parallel}$}, we consider %that 
 each term in Eq.\eqref{eq:def_J} %represents 
...
\label{eq:def_kxy}
\end{equation}
where $\langle J_{\mathrm{E}}^{\parallel} \rangle_T$ represents the thermal average at %a 
temperature $T$, and $L'$ is the 
the circumferential length of the cylinder
%length along the circumferential direction of the cylinder 
...
In this section, we explain the tensor network method %used 
...
bond dimension $D$ [See Fig.~\ref{fig:XTRG}(a)]. The string of the MPO is arranged in a snake form, %as shown in
as indicated by the gray line in Fig.~\ref{fig:lattice} (a). 

To optimize the tensors in an MPO as the density matrix at $\beta$, 
we employ the exponential thermal tensor renormalization group (XTRG) approach \cite{Chen2018,Li2020}, which has been successfully used to calculate %calculated 
...
In the following, we mainly show the data with $D=500$ for $(L, L') = (6, 6)$ cylinder. Several benchmark calculations for different values of $D$ and %a 
different lattice shapes %will be 
...
which enables us to calculate the finite temperature properties of quantum many-body systems using the power method. We note that several similar methods were independently proposed~\cite{Imada_JPSJ1986,Jaklic_PRB1994,Hams_PRE2000,Lloyd} before the proposal of the cTPQ method~[\onlinecite{Sugiura_PRL2013}].
We construct the 
...
In the calculation of the thermal average, the integral $\int \prod_i d\phi_i d\theta_i \sin\theta_i$ is evaluated %by 
...
%\blue{
The temperature derivative of $\langle O \rangle$ is evaluated by the correlation with ${\cal H}$ as  $d\langle O \rangle/dT=\left(\langle O %A
...
\end{align}
where $p$ runs over all hexagons in the honeycomb lattice and $N_{\rm h}$ is the number of hexagons. $\gamma_{i}$ %represents 
...
%We see 
we %find 
observe that clear double-peak structures %for each magnetic field which is considered to be 
appear at each magnetic field, which is a characteristic %behavior 
...
the specific heat exhibits non-smooth temperature dependence at low temperatures. The origin of this non-smooth behavior might be the effects of originate from the finite bond dimensions. However, the magnetic-field dependence of the low-temperature peak seems to be consistent with the %privious 
previous studies~\cite{YoshitakeNKM2020,LiZWWGQLGL2021,LiLXGQLS2024}, where the low-temperature peak moves toward a lower temperature %by 
under a weak magnetic field, while it %moves 
shifts toward a higher temperature as we increase the %magnec filed 
...
the magnetic moment monotonically increases, %and it reaches
...
We note that there is a phase transition between a %the 
...
with decreasing temperature, starting from $\langle J_{\mathrm{E}}^{\parallel} \rangle_T %J_{E}^{\parallel}
=0$ in the high-temperature limit. This behavior indicates that $\kappa_{xy}=d \langle J_{\mathrm{E}}^{\parallel} \rangle_T %J_{\mathrm{E}}^{\parallel}
/d T$ 
is positive, %and 
...
The dominant contribution of the edge current %also 
indicates that the nonzero thermal Hall conductivity %\blue{reflects} %is dominated by
originates from
the topological nature of the excitation structure. %properties. 
...
shift to %a 
higher temperatures side with increasing %as we increase
$h$.
...
%\red{削除しました}
Interestingly, $\kappa_{xy}/T$ becomes much larger than the half-quantized value (%shown 
...
which 
treated the magnetic fields as the effective three-body interactions~\cite{NasuYM2017}, or defined energy current in a simplified way~\cite{KumarT2023}. %\red{ここで、Kumarの計算にも言及した方が良い気がしたのですが、いかがでしょうか。} 
...
% we calculate the
contributions from the three-body and the two-body correlations %as shown 
...
In contrast to this, the %contributions 
...
magnons.
The low-field behavior is consistent with the Majorana fermion picture where the topological property %becomes rigid 
is stabilized by an applied magnetic field, but it cannot be accounted for by the magnon picture where the absolute value of $\kappa_{xy}/T$ should monotonically decrease %by applying 
...
across different directions, %while 
...
These results suggest that the Majorana fermion picture %is rather plausible 
...
From the symmetry of the Hamiltonian, we can explicitly show that $\kappa_{xy}$ becomes zero. The Hamiltonian is invariant under the following two simultaneous operations: %simultaneously:
