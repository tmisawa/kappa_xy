...
\red{Our comprehensive analyses indicate that the enhanced thermal Hall response is consistently explained by dominant contributions from topological Majorana fermions, even within the polarized regime beyond the quantum critical point.} 
Our %findings 
\red{approach not only} establish\red{es} a %comprehensive 
\red{robust} theoretical framework for understanding the thermal Hall transport in Kitaev materials such as $\alpha$-RuCl$_{3}$\red{, %and provide key insights into the detection of exotic quasiparticles in quantum spin liquids. 
%Furthermore, our approach 
but also offers a promising pathway to} bridg\red{e} %s 
the gap between %the theoretical predictions and experimental observations %, providing a robust foundation for exploring thermal transport in 
\red{theories and experiments across} a wide range of strongly correlated materials.
\end{abstract}
\maketitle
...
\red{We also investigate} the effects of $\Gamma$ and $\Gamma^\prime$, %we demonstrate 
\red{showing} tha\red{t %depending on the sign and type of 
t}hese interactions % 
\red{significantly affect} the thermal Hall conductivit\red{y %can vary 
i}n both \red{its} magnitude and sig\red{n.} %at finite temperatures. 
...
This insight} would contribute to a %better 
...
%B
b}y taking the derivative of $J_{\mathrm{E}}^{\parallel}$ with respect to temperature, %we obtain the thermal Hall conductivity 
\red{expressed} as 
\begin{equation}
...
%Temperature derivative of $J_{\mathrm{E}}^\parallel$ in Eq.~\eqref{eq:def_kxy} corresponds to the situation where a temperature gradient is applied perpendicular to the zigzag chains and $J_{\mathrm{E}}^\parallel$ is well localized at the edges [Fig.~\ref{fig:ThermalHall}(b)].
...
Specifically, the peak shifts to lower temperatures under weak magnetic fields, and moves to higher temperatures as the magnetic field increases beyond $%h_c 
\red{h} \gtrsim 0.02$.
This behavior is likely associated with the quantum phase transition in the Kitaev model, which separates a low-field topological chiral spin liquid from a high-field polarized state at $h_c \simeq 0.02\red{4}$\red{~\cite{Gohlke2018}}. %~\cite{Jiang2011}. 
...
%, and 
\red{This} also support\red{s} %ing 
...
Thus, our results indicate that full quantum calculations\red{,} using a proper definition of the energy current\red{, successfully} reproduce the overshooting behavior \red{observed in a wide range of magnetic fields in %agreement with 
experiments}. 
This \red{finding} suggests that the %experimental 
\red{interesting} featur\red{e} %s 
can be %explained purely by 
...
%Therefore, 
\red{These observations collectively suggest that} $\kappa_{xy}^{(L)}$ %is expected to primarily 
\red{predominantly} reflec\red{t %the 
c}ontributions from the topological gap opening in the emergent Majorana fermion system\red{, whereas} %.
%In contrast, the two-spin terms in Eq.~\eqref{eq:M} scale linearly with the magnetic field, suggesting that 
$\kappa_{xy}^{(M)}$ \red{may} includ\red{e} %s 
effects beyond the Majorana fermion picture.
%Since 
\red{Given that} $\kappa_{xy}$ is dominated by $\kappa_{xy}^{(L)}$\red{, %as discussed above, 
i}ts behavior in the calculated field range %is consistently understood by 
